...
class RigidBodySimulator(torch.nn.Module):
    def __init__(self, options):
        ...
        for i in range(self.frames * self.substep):
            ...
            self.vertices_sdf.append(torch.zeros(self.mesh.vertices.shape[0], dtype=torch.float32, requires_grad=True))
            self.vertices_sdf_grad.append(torch.zeros(self.mesh.vertices.shape[0], 3, dtype=torch.float32, requires_grad=True))
            self.collision_normal.append(torch.zeros(3, dtype=torch.float32, requires_grad=True))
        ...
        self.init_v = torch.tensor([0.0, 0.0, 5.0], dtype=torch.float32, requires_grad=True)
    ...
    def forward(self, f:torch.int32):
        ...
        planar_contact_normal = torch.tensor([0.0, 1.0, 0.0]) # contact normal (planar contact)
        d = torch.einsum('bi,i->b', xi, planar_contact_normal)
        rel_v = torch.einsum('bi,i->b', vi, planar_contact_normal)
        ...
        if num_collision > 0:
            ...
            v_i_n = vi.dot(planar_contact_normal) * planar_contact_normal
            v_i_t = vi - v_i_n
            vn_new = -self.kn * v_i_n
            # alpha = 1.0 - (self.mu * (1.0 + self.kn) * (torch.norm(v_i_t) / (torch.norm(v_i_n) + 1e-6)))
            # if alpha < 0.0:
            #     alpha = 0.0
            vt_new = (1 - self.mu) * v_i_t
