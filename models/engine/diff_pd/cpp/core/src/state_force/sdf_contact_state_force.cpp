#include "state_force/sdf_contact_state_force.h"

// what does p_ mean?
// p_ is a positive integer that controls the stiffness of the contact force.
// when the p_ is larger than 1:
// the value of std::pow(-d, p_ - 1) will enlarge the negative distance (value of d)
// which means with the same overlap distance d (negative), this case will have a larger contact force than the case of p_ = 1.
// when the p_ is 1:
// the value of std::pow(-d, p_ - 1) becomes 1, which means the state force fn_mag is linear to the negative distance d.
template<int vertex_dim>
void SDFContactStateForce<vertex_dim>::Initialize(const std::vector<real> sdf, 
    const std::vector<Eigen::Matrix<real, vertex_dim, 1>>& sdf_grad, const real offset,
    ...
    sdf_ = sdf;
    sdf_grad_ = sdf_grad;
    CheckError(sdf_.size() == sdf_grad_.size(), "Inconsistent sdf and sdf_grad.");
...
template<int vertex_dim>
const VectorXr SDFContactStateForce<vertex_dim>::ForwardForce(const VectorXr& q, const VectorXr& v) const {
...
template<int vertex_dim>
void SDFContactStateForce<vertex_dim>::BackwardForce(const VectorXr& q, const VectorXr& v, const VectorXr& f,
...
template class SDFContactStateForce<2>;
template class SDFContactStateForce<3>;