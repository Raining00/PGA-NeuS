...
// params: [offset, p, kn, kf, mu]
template<int vertex_dim, int element_dim>
void Deformable<vertex_dim, element_dim>::AddSDFStateForce(const std::vector<real>& params, const std::vector<real>& sdf,
        const std::vector<Eigen::Matrix<real, vertex_dim, 1>>& sdf_grad)
{
    CheckError(params.size() == 5, "Inconsistent params for SDFContactStateForce.");
    CheckError(sdf.size() == dofs() / vertex_dim, "Inconsistent sdf and dofs.");
    CheckError(sdf.size() == sdf_grad.size(), "Inconsistent sdf and sdf_grad.");
    const real offset = params[0];
    const int p = static_cast<int>(params[1]);
    const real kn = params[2];
    const real kf = params[3];
    const real mu = params[4];
    auto force = std::make_shared<SDFContactStateForce<vertex_dim>>();
    force->Initialize(sdf, sdf_grad, offset, p, kn, kf, mu);
    state_forces_.push_back(force);
}
